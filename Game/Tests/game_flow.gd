...
func run():
    ...
    var solved = current_scene.get_node("%SolvedPanel")
    check(solved.IsOpen, "Completion opens the solved panel")
    check(solved.get_global_rect().has_point(current_scene.get_node("%NumberBar").get_global_rect().get_center()), "Solved panel covers the controls")
    await capture("solved")
    solved.emit_signal("NewPuzzleRequested")
    await settle()
    board = current_scene.get_node("%Board")
    check(not current_scene.get_node("%SolvedPanel").IsOpen and board.GetValueCounts().size() == 10, "New puzzle from the solved panel opens a fresh game")
    check(not board.CanUndo, "New puzzle from the solved panel starts with fresh history")
    board.emit_signal("Solved")
    await create_timer(1.0).timeout
    current_scene.get_node("%SolvedPanel").emit_signal("MenuRequested")
    await settle()
    check(current_scene.scene_file_path.ends_with("StartScreen.tscn"), "Menu from the solved panel returns to the start screen")
